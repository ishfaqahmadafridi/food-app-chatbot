...
import { uploadProfileImage } from '../../../redux/features/profile/profileImageThunk/profileImageThunk';
import { clearImagePreview } from '../../../redux/features/profile/profileImageThunk/profileImageSlice';

const ImagePreviewModal = ({ imagePreview, isDisabled, onCancel, onUserUpdate }) => {
  const dispatch = useDispatch();
  const { error } = useSelector((state) => state.image);
  ...
  const handleConfirmUpload = async () => {
    if (imagePreview) {
      ...
      await dispatch(uploadProfileImage(imagePreview))
        .unwrap()
        .then((response) => {
